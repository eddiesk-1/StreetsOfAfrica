// server.js — Streets of Africa (in-memory demo)
// Express + express-fileupload + Socket.IO
...
// in-memory storage
let services = []; // newest first
let messages = []; // { from, to, text, time, read }

// username validation: name@10digits
const usernameRe = /^[a-zA-Z0-9_]+@[0-9]{10}$/;

// middleware
app.use(express.json());
app.use(express.urlencoded({ extended: true }));
app.use(fileUpload({ limits: { fileSize: MAX_BYTES }, abortOnLimit: true }));
app.use(express.static(path.join(__dirname, 'public')));

// helper: allowed mime map
function allowedExtForMime(mime) {
  const map = {
...
app.post('/api/upload', (req, res) => {
  try {
    ...
    const ext = allowedExtForMime(file.mimetype);
...
// delete service (demo: no auth)
app.post('/api/delete', (req, res) => {
  const { id } = req.body;
  services = services.filter(s => s.id !== id);
  io.emit('service-removed', { id });
  res.json({ ok: true });
});
